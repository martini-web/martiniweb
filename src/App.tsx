...
function InnerApp() {
  const [isLoginClicked, setIsLoginClicked] = useState(false); // Track if login button is clicked

  const login = useGoogleLogin({
    onSuccess: (tokenResponse) => {
      console.log('Login Success:', tokenResponse);
      window.location.href = redirectUri; // Redirect after login success
    },
    onError: (error) => {
      console.log('Login Failed', error);
    },
  ...
  const handleLoginClick = () => {
    setIsLoginClicked(true); // Hide the button after the click
    ...
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', width: '100vw' }}>
      <div style={{ marginTop: 100, textAlign: 'center' }}>
        {!isLoginClicked ? (
          ...
          <p>Redirecting to Google...</p> // Show a message after the button click
        )}
      </div>
    </div>
  );
